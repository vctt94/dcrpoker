...
// 1. Player 1 has Two Pair (10s and 7s) - rankValue=2954
// 2. Player 2 has Full House (7s over Qs) - rankValue=253
// 3. Player 2 should win (lower rankValue = better hand)
// 4. But Player 1 incorrectly wins due to pot restoration bug
func TestShowdownRestoreBug_HandEvaluationCorrectness(t *testing.T) {
	...
	// Join second player
	_, err = boot1.lc.JoinTable(ctx, &pokerrpc.JoinTableRequest{
		TableId:  tableID,
		PlayerId: p2,
	})
	require.NoError(t, err)
	...
	// Start game stream for player1
	stream1, err := boot1.pc.StartGameStream(ctx, &pokerrpc.StartGameStreamRequest{
		TableId:  tableID,
		PlayerId: p1,
	})
	require.NoError(t, err)

	// Start game stream for player2
	stream2, err := boot1.pc.StartGameStream(ctx, &pokerrpc.StartGameStreamRequest{
		TableId:  tableID,
		PlayerId: p2,
	})
	require.NoError(t, err)
	...
	var gameUpdate1, gameUpdate2 *pokerrpc.GameUpdate
	for {
		state1, err := stream1.Recv()
		require.NoError(t, err)
		if state1 != nil {
			gameUpdate1 = state1
			break
	...
	// Verify we're in PRE_FLOP
	require.Equal(t, pokerrpc.GamePhase_PRE_FLOP, gameUpdate1.Phase)
	require.Equal(t, pokerrpc.GamePhase_PRE_FLOP, gameUpdate2.Phase)

	// Capture own hole cards from each stream snapshot
	getOwnHole := func(update *pokerrpc.GameUpdate, pid string) []*pokerrpc.Card {
	...
	p1HolePB := getOwnHole(gameUpdate1, p1)
	p2HolePB := getOwnHole(gameUpdate2, p2)
	require.Len(t, p1HolePB, 2, "player1 should see own 2 hole cards")
	require.Len(t, p2HolePB, 2, "player2 should see own 2 hole cards")

    // Preflop actions will be driven dynamically below

    // Drive actions dynamically based on current player, mirroring other e2e tests
    ...
    // FLOP: current checks, then next checks → TURN
    st, _ = boot1.pc.GetGameState(ctx, &pokerrpc.GetGameStateRequest{TableId: tableID})
    cur = st.GameState.GetCurrentPlayer()
    ...
    // TURN: current checks, then next checks → RIVER
    st, _ = boot1.pc.GetGameState(ctx, &pokerrpc.GetGameStateRequest{TableId: tableID})
    cur = st.GameState.GetCurrentPlayer()
    _, err = boot1.pc.CheckBet(ctx, &pokerrpc.CheckBetRequest{TableId: tableID, PlayerId: cur})
    require.NoError(t, err)
    ...
    // Wait for RIVER and capture the board
    waitPhase(pokerrpc.GamePhase_RIVER)
    riverUpdate1, err := boot1.pc.GetGameState(ctx, &pokerrpc.GetGameStateRequest{TableId: tableID})
    require.NoError(t, err)

    boardPB := riverUpdate1.GameState.CommunityCards
	require.Len(t, boardPB, 5, "board should have 5 cards on river")
	...
	stream1Restored, err := boot2.pc.StartGameStream(ctx, &pokerrpc.StartGameStreamRequest{
		TableId:  tableID,
		PlayerId: p1,
	})
	require.NoError(t, err)

	stream2Restored, err := boot2.pc.StartGameStream(ctx, &pokerrpc.StartGameStreamRequest{
		TableId:  tableID,
		PlayerId: p2,
	})
	require.NoError(t, err)

	// Wait for restored game state
	var restoredState1, restoredState2 *pokerrpc.GameUpdate
	for {
		state1, err := stream1Restored.Recv()
		require.NoError(t, err)
		if state1 != nil {
			restoredState1 = state1
			break
		}
	}

	for {
		state2, err := stream2Restored.Recv()
		require.NoError(t, err)
		if state2 != nil {
			restoredState2 = state2
			break
		}
	}

	// Verify we're still in RIVER phase
	require.Equal(t, pokerrpc.GamePhase_RIVER, restoredState1.Phase)
	require.Equal(t, pokerrpc.GamePhase_RIVER, restoredState2.Phase)

    // 6) Complete the hand on RIVER using current player order (post-restore)
    stR, err := boot2.pc.GetGameState(ctx, &pokerrpc.GetGameStateRequest{TableId: tableID})
    require.NoError(t, err)
    curR := stR.GameState.GetCurrentPlayer()
    require.NotEmpty(t, curR, "restored game should have a current player on RIVER")
    _, err = boot2.pc.CheckBet(ctx, &pokerrpc.CheckBetRequest{TableId: tableID, PlayerId: curR})
    require.NoError(t, err)
    // Wait for either other's turn on RIVER or SHOWDOWN already
    other := p1
    if curR == p1 {
        other = p2
    }
	...
	// 7) Wait for showdown to complete
	require.Eventually(t, func() bool {
		st, err := boot2.pc.GetGameState(ctx, &pokerrpc.GetGameStateRequest{TableId: tableID})
		return err == nil && st.GameState.GetPhase() == pokerrpc.GamePhase_SHOWDOWN
	}, 3*time.Second, 25*time.Millisecond)

	// 8) Get showdown results and verify winners match evaluator
	winners, err := boot2.pc.GetLastWinners(ctx, &pokerrpc.GetLastWinnersRequest{TableId: tableID})
	require.NoError(t, err)
	switch cmp {
	case 0:
		// Tie: expect two winners (order not guaranteed)
		require.Len(t, winners.Winners, 2, "expected a split pot tie with 2 winners")
		got := map[string]bool{}
		for _, w := range winners.Winners {
			got[w.PlayerId] = true
		}
		require.True(t, got[p1] && got[p2], "both players should be winners on tie")
	case 1:
		// Player1 better
		require.GreaterOrEqual(t, len(winners.Winners), 1)
		require.Equal(t, p1, winners.Winners[0].PlayerId, "player1 should win per evaluator")
	case -1:
		// Player2 better
		require.GreaterOrEqual(t, len(winners.Winners), 1)
		require.Equal(t, p2, winners.Winners[0].PlayerId, "player2 should win per evaluator")
	}
}
