...
func TestStartGameStream_DisconnectRemovesBucket(t *testing.T) {
    ...
    wg.Add(2)
    go func() { defer wg.Done(); _ = s.StartGameStream(&pokerrpc.StartGameStreamRequest{TableId: tbl.GetConfig().ID, PlayerId: p1}, ms1) }()
    go func() { defer wg.Done(); _ = s.StartGameStream(&pokerrpc.StartGameStreamRequest{TableId: tbl.GetConfig().ID, PlayerId: p2}, ms2) }()
